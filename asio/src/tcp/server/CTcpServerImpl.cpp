...
CTcpServer::Impl::Impl() : m_ioContext(), m_acceptor(m_ioContext) {}

/* 析构时调用 stop() 确保线程安全退出；stop() 内部有状态守卫，重复调用无害。*/
CTcpServer::Impl::~Impl() { stop(); }
...
**
** 关键流程：
**   1. acceptor 配置（open / reuse_addr / bind / listen）——
* 短路求值，任一失败即终止
...
bool CTcpServer::Impl::listen(uint16_t port) noexcept {
  ...
  if (!startPool(m_ioThreadCount, m_ioContext, m_ioWorkGuard, m_ioThreads) //
      || !startPool(m_taskThreadCount, m_taskContext, m_taskWorkGuard,
                    m_taskThreads) //
  )
    return false;
...
** 每条线程内部捕获 std::exception 和 ... 两层，确保异常不逃逸到
* std::terminate。
** ───────────────────────────────────────────────────────────────────────── */
...
**
** 关键流程（顺序不可乱）：
**   1. acceptor 取消 + 关闭，触发 onAccept 以 operation_aborted 结束循环，释放
* shared_ptr<Impl>
**   2. O(1) 原子接管 session 映射
**   3. 关闭所有 session（异步 shutdown+close 投递到 writeStrand）
**   4. reset IO work_guard，让 IO 自然排空（所有 close/read 回调结束后 run()
* 返回）
...
**
** !! 不调用 ioContext.stop() !!
**    stop() 会遗弃仍持有 shared_ptr<Impl> 的 pending handler（如 onAccept
* lambda），
...
void CTcpServer::Impl::stop() noexcept {
  ...
  // ② 取消/关闭 acceptor；onAccept lambda 收到 operation_aborted
  // 后检查状态退出循环，
  ...
  // ④ 对每个 session 投递异步 shutdown+close；socket 关闭后 async_read_some
  // 以错误完成，
  //   doRead 退出，readStrand 上无更多工作
  for (auto &kv : sessions)
    kv.second->close();

  // ⑤ 只 reset work_guard，不调用 ioContext.stop()——让 IO context
  // 自然排空后退出；
  ...
  //   自然断开的 session 已在 onSessionDisconnected→postTask 路径触发（⑥ drain
  //   时执行）
...
bool CTcpServer::Impl::send2c(const NetAddr &clientAddr,
...
/* setCallback 投递到任务线程执行，保证 m_callback
 * 只在单一线程写入，无需加锁。*/
...
void CTcpServer::Impl::setCallback(ServerCallback &&callback) noexcept {
  postTask([cb = std::move(callback)](Impl &self) {
...
std::shared_ptr<CTcpSession>
CTcpServer::Impl::createSession(tcp::socket socket) noexcept {
  return make_shared_noexcept<CTcpSession>(std::move(socket), m_ioContext,
                                           this);
}
...
std::shared_ptr<CTcpSession>
CTcpServer::Impl::getSession(const NetAddr &addr) const noexcept {
...
void CTcpServer::Impl::onSessionMessage(const NetAddr &addr,
...
** 警告：postTask内的self为弱指针，不能执行异步操作或访问成员变量；仅适合执行纯计算或触发回调等不涉及 Impl 生命周期的任务。
** ───────────────────────────────────────────────────────────────────────── */
